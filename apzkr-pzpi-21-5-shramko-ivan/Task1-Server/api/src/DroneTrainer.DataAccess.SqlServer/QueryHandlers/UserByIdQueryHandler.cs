...
internal sealed class UserByIdQueryHandler(DroneTrainerDbConnection dbConnection) : IRequestHandler<UserByIdQuery, User>
{
    ...
    public async Task<User> Handle(UserByIdQuery request, CancellationToken cancellationToken)
    {
        ...
        var paramters = new SqlParameter("@Id", request.Id);
